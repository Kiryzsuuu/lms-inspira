      ...
      <div className="min-h-screen" style={{ background: '#F7F8FA' }}>
        ...
        <Container className="py-6">
          <div className="grid gap-6 lg:grid-cols-3">
            {/* Sidebar: module accordion (or flat list when no modules) */}
            <div className="lg:col-span-1 space-y-2">
              {lessons.length === 0 ? (
                ...
                <div className="overflow-hidden" style={{ border: '1px solid #E5E7EB', borderRadius: 14 }}>
                  {lessons.map((l, i) => {
                    const allowed = canOpenLessonByIndex(i);
                    const completed = lessonProgress[String(l._id)]?.isCompleted;
                    const isSelected = selectedLesson?._id === l._id;
                    return (
                      <button
                        key={l._id}
                        onClick={() => { if (allowed) handleSelectLesson(l, i); }}
                        disabled={!allowed}
                        className="w-full px-4 py-2.5 text-left text-sm flex items-center gap-3 transition-colors"
                        style={{
                          background: isSelected ? '#F0F8FD' : 'transparent',
                          color: !allowed ? '#9CA3AF' : isSelected ? '#0C628D' : '#374151',
                          cursor: !allowed ? 'not-allowed' : 'pointer',
                          fontWeight: isSelected ? 600 : 400,
                          borderTop: i > 0 ? '1px solid #F3F4F6' : 'none',
                        }}
                        onMouseEnter={(e) => { if (allowed && !isSelected) e.currentTarget.style.background = '#F9FAFB'; }}
                        onMouseLeave={(e) => { if (!isSelected) e.currentTarget.style.background = 'transparent'; }}
                      >
                        <div className="w-5 h-5 shrink-0 flex items-center justify-center">
                          {completed ? (
